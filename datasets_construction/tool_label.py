...
for file in os.listdir(data_dir):
    if file.endswith(".wav") and "_label" not in file:
        ...
        # 获取用户输入（新增删除标签3）
        label = input(f"Enter label for {file} (1=cat, 0=none, 2=cat+noise, 3=delete_immidiately): ")
        # label = input(f"Enter label for {file} (1=cat, 0=none, 2=cat+noise, 3=delete_immidiately, 4=play_audio_again): ")
